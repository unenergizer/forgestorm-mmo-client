...
public class PagedItemStackWindow extends HideableVisWindow implements Buildable {
    ...
    public PagedItemStackWindow(String windowTitle, int entryWidth, int entryHeight) {
        super(windowTitle);
        this.entryWidth = entryWidth;
        this.totalSize = entryWidth * entryHeight;
    }

    @Override
    public Actor build(final StageHandler stageHandler) {
        this.stageHandler = stageHandler;
        PagedItemStackWindow pagedItemStackWindow = this;
        TableUtils.setSpacingDefaults(this);
        addCloseButton();
        setResizable(false);

        pagedItemStackWindow.add(pageDisplay).row();
        ...
        pagedItemStackWindow.add(pageContainer).row();
        pagedItemStackWindow.add(navTable).expand().fill();

        previousPage.addListener(new ChangeListener() {
            @Override
            public void changed(ChangeEvent event, Actor actor) {
                Valenguard.getInstance().getAudioManager().getSoundManager().playSoundFx(PagedItemStackWindow.class, (short) 0);
    ...
    private List<VisTable> buildShopPage(short shopID) {
        ...
        for (int i = 0; i < entityShopWindowSlots.size(); i++) {
            ...
            if (pageCount >= totalSize) {
                ...
                pageCount = 0;
                columnCount = 0;
            } else if (columnCount == entryWidth) {
        ...
        int blankSpots = (shopPageList.size() * totalSize) - entityShopWindowSlots.size();
        columnCount = entityShopWindowSlots.size() % entryWidth;
    ...
    private void resetShop() {
        ...
        previousPage.setDisabled(true);
        nextPage.setDisabled(false);
        getTitleLabel().setText(getTitleLabel().getText());
    }
     ...
     * within a {@link PagedItemStackWindow}
     */
