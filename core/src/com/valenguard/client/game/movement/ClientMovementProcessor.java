...
public class ClientMovementProcessor {
    ...
    public void letOffAllKeys(PlayerClient playerClient) {
        playerClient.setPredictedMoveDirection(MoveDirection.NONE);
    }

    private void startNewMove(PlayerClient playerClient) {

        // Since the keybaord is the type of input and the player is stopped with will
    ...
    private void continueMove(PlayerClient playerClient) {
        ...
        if (inputData.getMovementInput() == MovementInput.KEYBOARD) {
            ...
            if (currentMovementInput == MovementInput.MOUSE) {
                ...
                if (predictedDirection == MoveDirection.NONE) {
                    throw new RuntimeException("Move direction was NONE when it is not suppose to be.");
                }
    ...
    // because it seems legit to put enums in your class
    ...
    public Queue<MoveNode> getNodeForDirection(PlayerClient playerClient, Location currentPlayerLocation, MoveDirection direction) {
        Queue<MoveNode> nextNode = new LinkedList<MoveNode>();
        Location futureLocation = MoveUtil.getLocation(playerClient.getTmxMap(), direction).add(currentPlayerLocation);
